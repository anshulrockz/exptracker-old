    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            <div class="header">
                ...
                <a class="btn btn-primary waves-effect header-dropdown m-r--5" href="{{ url('/employee-types/create')}}">Create New</a>
            </div>
            <div class="body">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                        ...
                        <tbody>
                        	@foreach( $employee_type as $key=>$list)
                            <tr>
                                <td>{{$list->name}}</td>
                                ...
                                <td>
                                    <a href="{{ url('/employee-types/'.$list->id)}}" class="btn btn-sm btn-success"> View </a>
                                    <a href="{{ url('/employee-types/'.$list->id.'/edit')}}" class="btn btn-sm btn-info"> Edit </a>
                                    <form style="display: inline;" method="post" action="{{route('employee-types.destroy',$list->id)}}">
				                        {{ csrf_field() }}
				                        {{ method_field('DELETE') }}
